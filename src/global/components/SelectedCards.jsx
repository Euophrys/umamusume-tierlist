    ...
    return (
        <div className="selected-cards font-sans text-left">
            <div className="flex items-center justify-between mb-2 border-b border-slate-100 dark:border-zinc-800 pb-2">
                ...
                <span className="text-xs text-slate-400 dark:text-zinc-500">{props.selectedCards.length} / 6 Cards</span>
            </div>
            
            <p className="text-xs text-slate-500 dark:text-zinc-400 mb-4 italic leading-relaxed">
                Click a card below to remove it, or click a card in the tier list to add it. Score calculations adapt automatically to your selected deck.
            </p>
            ...
            <div className="mt-3 p-3 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-850 rounded-lg space-y-2">
                <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500 dark:text-zinc-400 font-medium">Total Race Bonus:</span>
                ...
                <div className="text-[10px] text-slate-400 dark:text-zinc-500 leading-tight">
                    Aim for <span className="font-semibold text-slate-600 dark:text-zinc-400">35%</span> (URA/Unity) or <span className="font-semibold text-slate-600 dark:text-zinc-400">50%</span> (TB).
                </div>
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 dark:border-zinc-800">
                ...
                <div className="flex flex-wrap gap-1.5">
                    <button className="px-3 py-1 border border-slate-200 dark:border-zinc-800 rounded-full text-xs font-semibold bg-white dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors cursor-pointer" onClick={()=>props.onLoadPreset([30028,20023,20020,20009,20003])}>Speed/Power</button>
                    <button className="px-3 py-1 border border-slate-200 dark:border-zinc-800 rounded-full text-xs font-semibold bg-white dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-850 transition-colors cursor-pointer" onClick={()=>props.onLoadPreset([30028,20023,20020,20008,30022])}>Speed/Stamina</button>
                    <button className="px-3 py-1 border border-slate-200 dark:border-zinc-800 rounded-full text-xs font-semibold bg-white dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-850 transition-colors cursor-pointer" onClick={()=>props.onLoadPreset([30028,20023,20020,20012,20002])}>Speed/Wit</button>
                    <button className="px-3 py-1 border border-slate-200 dark:border-zinc-800 rounded-full text-xs font-semibold bg-white dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-850 transition-colors cursor-pointer" onClick={()=>props.onLoadPreset([30011,30030,30019,20012,20002])}>Guts/Wit</button>
                </div>
            </div>
        </div>
    );
